...
class TypeChecker : public parser::ast::visitor::IVisitor {
 public:
  TypeChecker();
 ...
 private:
  /*
   * converts a syntactic TypeNode into a semantic Type object.
   */
  std::shared_ptr<Type> ResolveSyntacticType(parser::ast::nodes::Type* node);
 ...
 private:
  std::shared_ptr<Type> last_type_; // type of the last visited expression
  std::shared_ptr<Type> current_expected_return_type_; // return type of current function
};
